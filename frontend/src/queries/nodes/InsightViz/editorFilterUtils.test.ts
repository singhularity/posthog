...
describe('editorFilterUtils', () => {
    ...
    describe('getSeriesSummary', () => {
        ...
        it('uses custom_name when available', () => {
            expect(getSeriesSummary([{ kind: NodeKind.EventsNode, custom_name: 'My Event', event: '$pageview' }])).toBe(
                'My Event'
            )
        })

        it('falls back to event name', () => {
            expect(getSeriesSummary([{ kind: NodeKind.EventsNode, event: '$pageview' }])).toBe('$pageview')
        })

        it('falls back to name when event is null', () => {
            expect(getSeriesSummary([{ kind: NodeKind.EventsNode, name: 'All events', event: null }])).toBe(
                'All events'
            )
        })
